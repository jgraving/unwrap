...
print(   list(`treat. kappa` =      summary(model_conc_kappa),     
              ...
              `no treat.` =      summary(model_conc_nodelta),
              `unwrapped rhat no treat.` =
                UnwrapRhats(model_nodelta,
                            variable = c('Intercept'),
                            regex = FALSE
...
As we might have also expected, the effect of treatment on $\mu$, present in our original model but not the $\kappa$-only model, does not improve predictive density, so there is no evidence that the treatment affects $\mu$.
