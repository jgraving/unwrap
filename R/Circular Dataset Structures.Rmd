...
Nonetheless, $\>95\%$ of estimates for change in 𝜇 are to the right of 0°, suggesting a change in direction.
...
This can be achieved with a student-T distribution with 3 degrees of freedom, a mean at 0, and a standard deviation of 0.5: $\sigma_\kappa \sim \mathrm{Half}\mathcal{T}(3,0,0.5)$.
...
prior_highcorr =  prior('normal(0,pi()/2)', class = 'b', nlpar = 'beta0') +
                  prior('unwrap_von_mises_vect(0, log1p_exp(softkappamu))',
                        nlpar  = 'betai',  class = 'b') +
                  # set_prior("target += normal_lpdf(softkappamu | 3, 3)"
...
dt_highcorr = data.frame(y = as.numeric( rad(unlist(dt_id1)) ),#angles as a numeric vector in radians
                         ID = factor(x = #individual labels as a unordered categorical (factor)
                                       sort(
                                         rep(1:length(dt_id1),
...
#random effects betai looks bad, but good after unwrapping
...
#        col = col_rho,
#        lwd = 7)
# with(draws_highcorr, paste0(mean(softplus(kappa_mu) > kappa_mu1)*100, '%') ) #overestimates through partial pooling
with(draws_highcorr,
