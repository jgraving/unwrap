...
Here, we simulate example data (𝑛 = 20) generated from a distribution with a mean differs that from the expected direction (0°) by only 15°.
...
A model fitted to the example data correctly indicates that the mean direction is to the left of the expected direction in \>99% of 𝜇 estimates.
...
```{r Model comparison observed vs expected mean angle}
...
round( deg(summary(mod_false)$fixed[1, 1:4]) )
...
```
Now the expected angle has somewhat higher predictive power, even though both find a similar mean direction (95% CI -7°–10° for `mod_false`).