...
stan_all = model_circular_fun + stan_unwrap_fun + 
...
prior_treat = prior('normal(0, 2*pi())',class = 'b', nlpar = 'pmu') + #wider prior helps avoid bias
  prior('unwrap_von_mises(0, 0.5)',class = 'b', nlpar = 'pmu', coef = 'treatment') + #expectation of moderate sized turns
...
print(lc_nulltreat)
#the treatment model has higher predictive power
#this could result from either a difference in mu or kappa
#to refine conclusions we would need to also fit a model for kappa
```
...
with(draws_treat,
     paste0(mean(b_imu_treatment > 0)*100, '%') #estimate somewhat favours a turn
)
